...
Shader "FantasyCrystals/TrippyCrystal2"
{
...
float totalSmoke = 0;
  ...
  newCol = pow( newCol , 3) * 10;
    ...
    col = col * .99 + newCol * hsv(float(i) * .1+ x ,1,1);//_SampleColor;///lerp( lerp(_BaseColor,_CenterOrbColor , saturate(centerOrbDensity)), _NoiseColor , saturate(noiseDensity));// saturate(dot(v.lightDir , nor)) * .1 *c;//hsv(c,.4, dT3D(p*3,float3(0,-1,0))) * c;//hsv(c * .8 + .3,1,1)*c;;// hsv(smoke,1,1) * saturate(smoke);
